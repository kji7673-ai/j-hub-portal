...
// 1. Find indices
let part3Index = bookData.pages.findIndex(
  (p) => p.title && p.title.includes("3부. 정비나침반 실행 로드맵"),
);
let part4Index = bookData.pages.findIndex(
  (p) => p.title && p.title.includes("4부. 건축가, 인간을 짓다"),
);

// 2. Rename Part 4 to Part 3
if (part4Index !== -1) {
  bookData.pages[part4Index].title = bookData.pages[part4Index].title.replace(
    "4부",
    "3부",
  );
...
console.log(
  "Restructured to Appendix successfully. Total pages:",
