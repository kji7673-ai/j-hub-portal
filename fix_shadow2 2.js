...
html = html.replace(
  /<p style="font-family: var\(--font-display, 'SF Pro Display', sans-serif\); font-size: 14px; font-weight: 600; color: var\(--primary-on-dark, #2997ff\); letter-spacing: 0.1em; margin-bottom: 16px; text-transform: uppercase;">\$\{part\}<\/p>/g,
...
html = html.replace(
  /<h1 class="book-title" style="font-family: var\(--font-display, 'SF Pro Display', sans-serif\); font-size: clamp\(28px, 6vw, 44px\); font-weight: 700; color: #ffffff; margin: 0 0 24px 0; line-height: 1.25; letter-spacing: -0.02em; word-break: keep-all;">\$\{mainTitle\}<\/h1>/g,
...
html = html.replace(
  /<p style="font-family: var\(--font-body, 'SF Pro Text', sans-serif\); font-size: 18px; font-weight: 300; color: #a0a0a0; margin: 0; word-break: keep-all; line-height: 1.5;">\$\{subtitle\}<\/p>/g,
