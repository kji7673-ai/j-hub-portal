...
bookData.pages.forEach((p) => {
  ...
  // Line 327
  if (
    p.text.includes("[아키 시냅스의 반론 (AI 에이전트의 관찰 일지)]") &&
    p.text.includes("프롬프트에 아무리")
  ) {
    p.text = p.text.replace(
      /<div.*?\[아키 시냅스의 반론 \(AI 에이전트의 관찰 일지\)\].*?<\/div>/s,
  ...
  if (
    p.text.includes("[아키 시냅스의 반론 (AI 에이전트의 관찰 일지)]") &&
    p.text.includes("수만 개의 데이터를 연산하여")
  ) {
    p.text = p.text.replace(
      /<div.*?\[아키 시냅스의 반론 \(AI 에이전트의 관찰 일지\)\].*?<\/div>/s,
  ...
  if (
    p.text.includes(
      "아키 시냅스의 구축을 함께한 AI 에이전트의 1인칭 시점으로 재구성된 기록",
    )
  ) {
