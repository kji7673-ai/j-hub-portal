...
for (let p of bookData.pages) {
  ...
  if (p.title === "Theme 1을 마치며") {
    bridges[1] = p;
    continue;
  }
  if (p.title === "Theme 2를 마치며") {
    bridges[2] = p;
    continue;
  }
  if (p.title === "Theme 3을 마치며") {
    bridges[3] = p;
    continue;
  }
  if (p.title === "2부를 마치며" && p.partCategory === "2부: 철학편") {
    bridges[4] = p;
    continue;
  }
  ...
  if (p.text) {
    p.text = p.text.replace(
      /\[J-Hub 코어: 아키 시냅스 시스템 로그\]/g,
      "J-Hub 코어: 아키 시냅스 시스템 로그",
    );
    p.text = p.text.replace(
      /<b>\[현장 스케치:\s*(.*?)\]<\/b>/g,
      '<strong style="color: var(--primary);">현장 스케치: $1</strong>',
    );
    p.text = p.text.replace(
      /\[현장 스케치:\s*(.*?)\]/g,
      '<strong style="color: var(--primary);">현장 스케치: $1</strong>',
...
for (let p of appendices) {
  if (p.partCategory.includes("부록 A") && !p.title.includes("부록 A"))
    p.title = "부록 A. " + p.title.replace("부록. ", "");
  if (p.partCategory.includes("부록 B") && !p.title.includes("부록 B"))
    p.title = "부록 B. " + p.title.replace("부록. ", "");
  if (p.partCategory.includes("부록 C") && !p.title.includes("부록 C"))
    p.title = "부록 C. " + p.title.replace("부록. ", "");
}
