...
function initLeadMagnet() {
  ...
  const target = targets.find((el) =>
    /사례|case|프롬프트|실전|예시/i.test(el.textContent),
  );
  ...
  try {
    ...
    if (document.getElementById("progressFill")) {
      ...
      document.getElementById("progressText").innerText =
        `${completedCount}개 완독 (${percentage}%)`;
    }
    ...
    let completedCount = Object.keys(progress).filter(
      (k) => progress[k] === true,
    ).length;
    let percentage =
      totalDocs > 0 ? Math.round((completedCount / totalDocs) * 100) : 0;
...
searchInput.addEventListener("input", (e) => {
  ...
  if (results.length === 0) {
    searchResults.innerHTML =
      '<div style="padding: 20px; text-align: center; color: var(--ink-muted);">검색 결과가 없습니다.</div>';
    return;
  }
  ...
  results.slice(0, 15).forEach((item) => {
    ...
    if (matchIdx > -1) {
      ...
      snippet =
        ...
        snippet.substring(matchIdx, matchIdx + query.length) +
        "</span>" +
    ...
    if (titleIdx > -1) {
      titleHtml =
        titleHtml.substring(0, titleIdx) +
        '<span class="highlight">' +
        titleHtml.substring(titleIdx, titleIdx + query.length) +
        "</span>" +
