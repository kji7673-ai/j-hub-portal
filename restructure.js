...
let finalPages = [];
let theme1Keep = ["찢어진 운동화", "삼켜낸 말과 술 한 잔", "가끔", "거울 앞의 건축가", "가면을 벗는 시간", "난 약한 사람입니다", "오뚝이"];
...
for (let i = 0; i < bookData.pages.length; i++) {
    ...
    // Filtering Theme 1 to core items
    if (p.partTitle && p.partTitle.includes("Theme 1")) {
        let keep = false;
        for (let t of theme1Keep) {
            if (p.title && p.title.includes(t)) {
                keep = true;
                break;
            }
        }
        if (keep || p.type.includes('image')) {
            finalPages.push(p);
        }
    } 
    else {
