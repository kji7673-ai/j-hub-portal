...
// old 4부 Ch 1~4
let old4buCh1_4 = pages.filter(
  (p) =>
    p.part === "4부: 비전과 확장" &&
    p.type !== "interlude" &&
    p.title.includes("제5장") === false,
);
old4buCh1_4.forEach((p) => {
  p.part = "1부: 프롭테크와 정비사업의 미래";
  newPages.push(p);
});

// old 1부 프롤로그
let old1buPrologue = pages.filter(
  (p) =>
    p.part &&
    p.part.includes("1부: 시스템편") &&
    p.title &&
    p.title.includes("프롤로그"),
);
old1buPrologue.forEach((p) => {
  p.part = "1부: 프롭테크와 정비사업의 미래";
  newPages.push(p);
});
...
// old 1부 remaining
let old1buRem = pages.filter(
  (p) =>
    p.part &&
    p.part.includes("1부: 시스템편") &&
    (!p.title || !p.title.includes("프롤로그")),
);
