...
function renderBook() {
  ...
  bookData.pages.forEach((page, index) => {
    ...
    if (page.type === "cover") {
      ...
      if (page.image)
        contentHTML += `<div class="image-container" id="img-container-${index}"><img src="${page.image}" alt="sketch"></div>`;
      if (page.title)
      ...
      if (page.text) {
        ...
        paragraphs.forEach((p) => {
          if (
            ...
            p.trim().startsWith('<div class="custom-table"')
          ) {
            contentHTML += p;
          } else {
      ...
      if (page.title)
        contentHTML += `<h1 class="book-title" style="text-align:center; color:#1d1d1f; font-size:48px; margin-bottom:20px; text-shadow: 0 2px 15px rgba(255,255,255,0.9); font-weight:700;">${page.title}</h1>`;
      if (page.subtitle)
        contentHTML += `<p style="text-align:center; color:#333; font-size:24px; font-weight:600; text-shadow: 0 1px 10px rgba(255,255,255,0.9);">${page.subtitle}</p>`;
      if (page.text) {
        const paragraphs = page.text.split("\n\n");
        paragraphs.forEach((p) => {
      ...
      if (page.title)
        contentHTML += `<h2 class="chapter-title">${page.title}</h2>`;
      if (page.text) {
        const paragraphs = page.text.split("\n\n");
        paragraphs.forEach((p) => {
          if (
...
function getChapterMaxColumns(chapterIndex) {
  ...
  if (!p) return 1;
  const w =
    p.clientWidth || document.getElementById("book-container").clientWidth;
...
function updateControls() {
  ...
  const w =
    ...
    document.getElementById("book-container").clientWidth;
