...
newPart3.push({
  ...
  title: "제1장. 신뢰를 기록하다",
  text:
    "J-Hub의 존재 이유는 '업무의 자동화'가 아니라 '투명성과 책임의 기록'에 있습니다. 우리는 2부의 에세이들을 통해 수많은 의심과 갈등이 빚어내는 비극을 보았습니다. 기계는 계산할 수 없는 마음들을 위로하기 위해, 역설적으로 가장 투명하고 차가운 데이터를 제시합니다. \n\n이 시스템은 묻습니다. 기술이 모든 것을 투명하게 보여줄 때, 그 투명성을 해석하고 설득하는 책임은 누구에게 있는가? 그것은 결국 현장에서 흙먼지를 마셔본 자의 몫입니다." +
    createAIPanel(
      '<b>아키 시냅스 (AI):</b><br>"나는 완벽하고 투명한 데이터를 실시간으로 제시합니다.<br>그러나 그 데이터를 현장의 맥락으로 해석하는 것,<br>그리고 그 결정으로 파생되는 눈물과 기쁨에 책임지는 것은 오직 당신(인간)입니다."',
    ),
  partCategory: "3부: 철학의 기술화",
});
...
newPart3.push({
  ...
  text:
    ch3Text +
    createAIPanel(
      '<b>아키 시냅스 (AI):</b><br>"나는 모든 가능한 시나리오와 발생할 수 있는 리스크를 빠짐없이 보여줄 수 있습니다.<br>하지만 최종적으로 도면 위에 선을 긋고, 그 무거운 결과에 책임을 지는 것은 언제나 당신입니다."',
    ),
  partCategory: "3부: 철학의 기술화",
...
if (techPhase) {
  techPhase.partCategory = "부록 A: J-Hub 기술 개요";
  newApp.push(techPhase);
}
if (techTwoTrack) {
  techTwoTrack.partCategory = "부록 A: J-Hub 기술 개요";
  newApp.push(techTwoTrack);
}
for (let p of part3Raw) {
  if (
    p.image &&
    (p.image.includes("timeline") ||
      p.image.includes("flowchart") ||
      p.image.includes("orchestra") ||
      p.image.includes("kanban"))
  ) {
