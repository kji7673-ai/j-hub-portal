...
if (page1_1) {
  page1_1.partCategory = part1Cat;
  newPages.push(page1_1);
}
...
if (page1_2_q1) {
  page1_2_q1.partCategory = part1Cat;
  newPages.push(page1_2_q1);
}
...
const theme3 = [
  "주변에 순응하라",
  "AI가 읽지 못하는 지역 맥락",
  "100년의 기억을 덮는다는 것의 무게",
  "북서향 집이다",
  "다닥 다닥 붙은 집들",
  "맥락 속의 완벽함 (Harmony, Not Perfection)",
  "내가 생각하는 디자인 1",
  "내가 생각하는 디자인 2",
  "방향성과 중심성",
  "스케일과 대비",
  ...
  "만지작거리고 바스락거린다.",
  "필연의 신은 누구인가요",
  "하지말아야 될것이 있는것에 익숙해져버렸다",
];
const theme4 = [
  "심의와 설득의 기술",
  "타당성 검토의 배신",
  "화려한 조감도 뒤에 가려진 씁쓸한 현실",
  "페이퍼 아키텍트의 불안",
  "오늘 건축 심의가 있는 날이다.",
  "심의전 자문받겠다해서",
  "설계를 하다보면 페이퍼 아키텍쳐라는...",
...
let p0 = findPageByTitle("제1장. 투명성의 약속");
if (p0) {
  p0.partCategory = part3Cat;
  p0.title = "제2장. 투명성의 약속 (투 트랙 시스템)";
  newPages.push(p0);
}
let p1 = findPageByTitle("제2장. 6가지 길");
if (p1) {
  p1.partCategory = part3Cat;
  p1.title = "제3장. 6가지 길 중 당신의 길 찾기";
  newPages.push(p1);
}
let p2 = findPageByTitle("제3장. 누가, 언제, 무엇을 하는가");
if (p2) {
  p2.partCategory = part3Cat;
  p2.title = "제4장. 누가, 언제, 무엇을 하는가?";
  newPages.push(p2);
}

let pA = findPageByTitle("챕터 A. 기술이 풀 수 없는 것들");
if (pA) {
  pA.partCategory = part3Cat;
  newPages.push(pA);
}
let pB = findPageByTitle("챕터 B. 10년 뒤 정비사업의 풍경");
if (pB) {
  pB.partCategory = part3Cat;
  newPages.push(pB);
}
let pC = findPageByTitle("챕터 C. 건축가의 존엄을 지키는 법");
if (pC) {
  pC.partCategory = part3Cat;
  newPages.push(pC);
}

// 에필로그
let ep1 = findPageByTitle("다시, 신발을 신다");
if (ep1) {
  ep1.partCategory = part3Cat;
  newPages.push(ep1);
}
...
oldPages
  .filter((p) => p.partCategory === "부록")
  .forEach((p) => {
    // Keep only some core appendix items to shorten as requested
    if (
      p.title &&
      (p.title.includes("QR") ||
        p.title.includes("마스터 프롬프트") ||
        p.title.includes("자가 진단"))
    ) {
