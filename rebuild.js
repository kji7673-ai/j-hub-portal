...
if (progIdx !== -1) {
  ...
  p.part = "1부: 프롭테크와 정비사업의 미래";
  if (pages[progIdx - 1].type === "image_full")
    newPages.push(pages[progIdx - 1]);
  newPages.push(p);
}
...
let part3InterludeIdx = pages.findIndex(
  (p) => p.type === "interlude" && p.title && p.title.includes("3부"),
);
...
for (let i = startPart2; i < part3InterludeIdx; i++) {
  ...
  if (
    pages[i].type === "interlude" &&
    pages[i].title &&
    (pages[i].title.includes("1부") || pages[i].title.includes("2부"))
  )
    continue;
  let p = Object.assign({}, pages[i]);
