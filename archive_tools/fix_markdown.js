...
function markdownTableToHtml(markdown) {
  ...
  let html =
    '<div class="custom-table" style="background: var(--canvas-parchment, #f5f5f7); border-radius: 11px; padding: 24px; margin-top: 20px; border: 1px solid var(--hairline, #e0e0e0); margin-bottom: 24px; overflow-x: auto; font-size: 14px;"><table style="width:100%; border-collapse: collapse;">';
  ...
  html +=
    ...
    headers
      .map(
        (h) =>
          `<th style="border-bottom: 2px solid var(--primary); padding: 10px; text-align: left; color: var(--ink);">${h}</th>`,
      )
      .join("") +
    "</tr></thead><tbody>";
  ...
  for (let i = 2; i < lines.length; i++) {
    ...
    html +=
      ...
      cells
        .map(
          (c) =>
            `<td style="border-bottom: 1px solid var(--hairline); padding: 10px; color: var(--ink-muted-80);">${c}</td>`,
        )
        .join("") +
      "</tr>";
  }
