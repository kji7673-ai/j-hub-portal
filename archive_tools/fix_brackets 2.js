...
for (let i = 0; i < data.pages.length; i++) {
  ...
  if (
    p.title ===
    "[Y구역 현장 기록 2] AI가 읽지 못하는 지역 맥락: 달동네의 바람길"
  ) {
    newPages.push({
      type: "cover",
      title: "Y구역 현장 기록",
  ...
  // Fix others
    ...
    p.title === "[디자인 철학] 내가 생각하는 디자인 2: 포용과 사이 공간"
  ) {
  ...
  // Add cover for Y구역 3,1 (they are sequential in Theme 4)
    ...
    p.title === "[Y구역 현장 기록 3] 완벽한 보고서의 패배: 심의와 설득의 기술"
  ) {
    newPages.push({
      type: "cover",
      title: "Y구역 현장 기록",
    ...
    p.title === "[Y구역 현장 기록 1] 서류 완벽주의의 함정: 타당성 검토의 배신"
  ) {
    ...
    p.title &&
    p.title.includes(
      "막간극(Interlude). [AI의 시선] 목적이 분명한 기획자는 어떻게 기계를 움직이는가",
    )
  ) {
