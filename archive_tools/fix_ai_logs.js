...
bookData.pages.forEach((p) => {
  ...
  p.text = p.text.replace(
    /border-left: 4px solid var\(--primary\);/g,
    "border-left: 4px solid #333333;",
  );
  p.text = p.text.replace(
    /border-left: 3px solid #0066cc;/g,
  ...
  if (p.text.includes("아키 시냅스 시스템 로그")) {
    ...
    if (p.text.includes("종전자산 3D 그래프")) {
      p.text = p.text.replace(
        /<div style="background-color: var\(--canvas-parchment\);.*<\/div>/s,
...
html = html.replace(
  /item.onmouseover = \(\) => \{ item.style.color = '#0071e3'; item.style.backgroundColor = '#f5f5f7'; \};/g,
  "item.onmouseover = () => { item.style.color = '#1d1d1f'; item.style.backgroundColor = '#f5f5f7'; font-weight: '600'; };",
);
