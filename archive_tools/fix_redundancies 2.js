...
bookData.pages = bookData.pages.filter((p) => {
  if (p.title === "11장. 엔터프라이즈 엘레강스 (Enterprise Elegance)")
    return false;
  if (p.title === "88. 과거의 기억을 현재의 목적에 맞추어 이용한다")
    return false;
...
let mindIndex = bookData.pages.findIndex(
  (p) => p.title && p.title.includes("쟁이의 마음:"),
);
if (mindIndex !== -1) {
  ...
  let part3InterludeIndex = bookData.pages.findIndex(
    (p) => p.type === "interlude" && p.title.includes("3부. 프롭테크와"),
  );
...
let closingQuote = bookData.pages.find(
  (p) => p.title && p.title.includes("마치는 글:"),
);
if (closingQuote) {
