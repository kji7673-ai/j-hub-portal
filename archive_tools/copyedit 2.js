...
for (let p of bookData.pages) {
  if (p.text) {
    ...
    p.text = p.text.replace(
      "내 손가락을 대신 움직여주지 않습니다",
      "내 손가락을 대신 움직여 도면을 그려주지 않습니다",
    );
    p.text = p.text.replace(
      "내가 더 빠르고 더 명확하게 생각하여, 그들에게 쏟을 시간을 벌어줄 뿐입니다",
      "나의 복잡한 연산을 덜어주어, 그들의 손을 맞잡고 설득할 귀중한 '인간의 시간'을 벌어줄 뿐입니다",
    );
    p.text = p.text.replace(
      "기계는 수익성 저하 경고를 띄웠지만",
      "시스템은 즉각 '수익성 저하' 경고등을 띄웠지만",
    );
  }
}
