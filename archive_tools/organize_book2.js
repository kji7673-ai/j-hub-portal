...
let coverPage = pages.find(
  (p) => p.type === "image_full" && p.title === "도면 위에 머무는 시간",
);
let authorPage = pages.find((p) => p.type === "author_profile");
let prologuePage = pages.find(
  (p) => p.title && p.title.includes("프롤로그: 완벽한 시스템"),
);
let bridgePage = pages.find((p) => p.title && p.title.includes("[브릿지]"));
let epilogue1 = pages.find(
  (p) => p.title && p.title.includes("에필로그: 26년의 경험"),
);
...
pages.forEach((p) => {
  ...
  if (p.type === "interlude") {
    if (p.title.includes("프롭테크") || p.title.includes("1부"))
      currentPart = "proptech";
    else if (p.title.includes("마스터플랜") || p.title.includes("2부"))
      currentPart = "manual";
    else if (p.title.includes("증언과 성찰") || p.title.includes("3부"))
      currentPart = "diary";
  ...
  if (currentPart === "proptech") proptechChapters.push(p);
    ...
    if (
      titleStr.includes("제1장") ||
      titleStr.includes("제2장") ||
      titleStr.includes("제3장") ||
      titleStr.includes("제4장")
    ) {
      proptechChapters.push(p);
    } else if (
      titleStr.includes("부록 세부항목") ||
      titleStr.includes("Part ")
    ) {
      appendixChapters.push(p);
    } else if (
      titleStr.includes("1장.") ||
      titleStr.includes("2장.") ||
      titleStr.includes("3장.") ||
      titleStr.includes("4장.")
    ) {
...
manualChapters.forEach((p) => {
  p.partCategory = "1부: 플랫폼 마스터플랜";
  finalPages.push(p);
});
