...
const tagRegex =
  /<div style=\\"background-color: var\(--surface-pearl[^>]*>.*?\[연결된 철학:[^\]]+\]\\n\s*<\/div><br>/g;
...
const tagRegex2 =
  /<div style=\\"background-color: var\(--surface-pearl.*?(?:\[연결된 철학:[^\]]+\]|\[연결된 철학:[^\]]+\]).*?<\/div><br>/g;
...
const tagRegex3 =
  /<div style=\\"background-color: var\(--surface-pearl, #fafafc\); padding: 12px 16px; border-radius: 8px; margin-bottom: 24px; border-left: 3px solid var\(--primary, #0066cc\); font-size: 0.9em; color: var\(--ink-muted-80, #333333\); font-weight: 600; display: inline-block;\\">\\n\s*<svg[^>]*>.*?<\/svg>\\n\s*\[연결된 철학:[^\]]+\]\\n\s*<\/div><br>/g;
