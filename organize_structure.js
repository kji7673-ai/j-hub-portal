...
function pullByKeyword(keyword) {
  const idx = bookData.pages.findIndex(
    (p) => p.title && p.title.includes(keyword),
  );
  if (idx !== -1) {
    return bookData.pages.splice(idx, 1)[0];
  }
...
// 1. Prologue Setup
prologue.push(
  pullByTitle("프롤로그: 건축 외에는 아무것도 모르는 바보의 이야기"),
);
...
part3.push(
  pullByTitle("막간극: 목적이 분명한 기획자는 어떻게 기계를 움직이는가"),
);
...
appendices.push(
  pullByTitle(
    "부록 A. [기계가 계산할 수 없는 마음] 진양건축의 26년 - 신뢰의 축적",
  ),
);
...
let remaining = bookData.pages.filter(
  (p) =>
    p !== null &&
    p.title &&
    p.title !== "No Title" &&
    !p.title.includes("마치며") &&
    !p.title.includes("테마"),
);
...
bookData.pages.forEach((p) => {
  ...
  if (p.title && p.title.includes("어느 날, 공유결합이")) {
    p.text = p.text.replace(
      '이 말에 그래 우린 몸도 이온과 공유결합으로 이루어 져 있는데, 나와 사람과의 관계 모든 관계성에 공유 겹합이 있구나, 하늘과 땅 그리고 사람. 사람은 "네~"라고 응당할수 있으며, 연결 짓고, 연결하는 창조자의 위치까지도 있구나 하는 생각든 든 날이였습니다. 그날 이후 전 "공유결합"이라는 메타포를 가지고 설계를 하고 있습니다. _ 이 내용이 어떤가요?',
      "이온 결합이 서로의 전자를 빼앗거나 주면서 결합한다면, 공유결합은 각자의 전자를 '공유'하여 가장 안정적인 상태를 이룹니다. 이 말에 문득 깨달음이 왔습니다. 아, 우리의 몸도, 건축도, 사람과 사람의 관계도 결국 서로의 것을 내어주고 공유하며 안정된 구조를 이루는 '공유결합'이구나. 하늘과 땅, 그리고 그 사이를 잇는 사람. '왜'가 아닌 '네'라고 응답하며 타인과 기꺼이 마음을 나누고 공간을 연결하는 창조자. 그날 이후, 저는 이 '공유결합'이라는 메타포를 가슴에 품고 제 모든 설계의 바탕으로 삼았습니다.",
    );
  }
  ...
  if (p.title === "공유결합의 완성") {
    p.text = p.text.replace(
      "이것으로 기나긴 증언과 성찰의 일지를 마칩니다.",
      "이 책에 담긴 26년의 궤적이, 차가운 콘크리트 속에서도 온기를 피워내는 진짜 건축을 꿈꾸는 이들에게 작은 위로와 영감이 되기를 바랍니다. 이것으로 기나긴 증언과 성찰의 일지를 마칩니다.",
    );
  }
});
...
console.log(
  "Reorganized and edited book_data.js successfully. New page count:",
