...
function renderBook() {
  ...
  bookData.pages.forEach((page, index) => {
    ...
    if (page.type === "cover" || page.type === "interlude") {
      ...
      pText = pText.replace(
        /<p style='(.*?)'>/g,
        "<p style='$1 text-shadow: 0 4px 15px rgba(0,0,0,0.8); color: rgba(255,255,255,0.9); font-weight: 300;'>",
      );
      ...
      if (page.image)
        contentHTML += `<div class="image-container" id="img-container-${index}" style="max-width: 800px; width: 90%; margin-bottom: 45px;"><img src="${page.image}" alt="sketch"></div>`;
      if (page.title)
