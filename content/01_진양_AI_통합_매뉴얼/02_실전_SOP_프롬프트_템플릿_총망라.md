...
<div class="episode ep-fail">
  <div class="ep-header"><div class="ep-icon">⚠️</div><span class="ep-title">검증 포인트: AI는 3D 지형을 모른다</span></div>
  <p>AI가 "면적 2,000㎡, 정형화된 평탄지"라고 대답했더라도, 로드뷰나 현장 답사를 통해 <strong>실제 대지의 경사도와 좁은 진입로 폭원</strong>을 반드시 수동으로 보정(Override)하여 다시 프롬프트를 날려야 합니다.</p>
</div>
...
<div class="alert alert-secondary" style="margin-top:16px;">
    <strong>💡 건축사의 교차 검증 (Cross-Check)</strong>
    <p>AI가 "주변 시세 10억, 분양성 양호"라고 판별하더라도, 대상지 바로 옆에 혐오시설(철도 소음, 대형 변전소)이나 골목 이면도로 상권이 있다면 분양가는 급락합니다. AI가 놓친 <strong>'정성적 환경 제약'</strong>을 찾아내어 프롬프트에 추가 조건으로 걸어주는 것이 게이트키퍼(건축사)의 역할입니다.</p>
</div>
...
<div class="alert alert-danger" style="margin-bottom: 20px;">
    <strong>[🚨 치명적 환각 TOP 3 치트키]</strong>
    <p>AI가 산출한 건축 개요를 볼 때, 아래 3가지는 <strong>반드시 법제처(law.go.kr) 원문과 직접 대조</strong>하십시오.</p>
    <ul>
        <li><strong>임대주택 완화 비율:</strong> 제2종일반주거지역의 임대주택 건립 시 용적률 완화 산식이 맞게 적용되었는가?</li>
        <li><strong>노후도 조건:</strong> 지자체별 노후도 충족 비율(예: 67% vs 57%)이 최신 고시 기준으로 반영되었는가?</li>
        <li><strong>주차 대수 산정:</strong> 서울시 주차장 조례 기점(세대당 1대 vs 1.2대)이 평형별로 정확히 쪼개어졌는가?</li>
    </ul>
</div>
...
<div class="episode ep-solve">
  <div class="ep-header"><div class="ep-icon">💡</div><span class="ep-title">최종 종합 의견 템플릿 (AI 입력용)</span></div>
  <p>"위 비교표 밑에 나의 종합 의견을 다음과 같이 추가해서 최종 보고서를 포맷팅해 줘.<br>
  [건축사 의견] Alt 1이 세대수는 많지만, 북측 경사지로 인한 흙막이 토목 공사비 증가분과 최근 하이엔드 분양 시장 트렌드를 감안할 때, 조경과 펜트하우스를 특화한 Alt 2 방식이 조합원 분담금 최소화에 유리할 것으로 판단됨."</p>
</div>
