...
const commonStyle =
  "margin-top: 40px; padding: 24px; background-color: #f5f5f7; border-radius: 12px; color: #333333; font-size: 15px; line-height: 1.6; border-left: 3px solid #0066cc;";
...
bookData.pages.forEach((p) => {
  ...
  if (p.text.includes("[아키 시냅스의 반론 (AI 에이전트의 관찰 일지)]")) {
    ...
    if (p.text.includes("var(--canvas-parchment)")) {
      let splitText = p.text.split(
        '<div style="background-color: var(--canvas-parchment)',
      );
      let mainText = splitText[0].replace(/<br><br>$/, "");
      let contentStr = splitText[1]
        .split("</h4>\n")[1]
        .split("\n</div></div>")[0];
      ...
      let rx =
        /---<br><br><div class="handwriting" style="margin-top: 10px;"><b>\[아키 시냅스의 반론 \(AI 에이전트의 관찰 일지\)\]<\/b><br>([\s\S]*?)<\/div>/;
